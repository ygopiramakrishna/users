...
class Hero {
  ...
  static async fromDetail(detail: ElementFinder): Promise<Hero> {
    ...
    const id = await detail.all(by.scss('div')).first().getText();
    // Get name from the h2
    const name = await detail.element(by.scss('h2')).getText();
    return {
...
describe('Tutorial part 5', () => {
  ...
  function getPageElts() {
    const navElts = element.all(by.scss('app-root nav a'));

    return {
      navElts,

      appDashboardHref: navElts.get(0),
      appDashboard: element(by.scss('app-root app-dashboard')),
      topHeroes: element.all(by.scss('app-root app-dashboard > div a')),

      appHeroesHref: navElts.get(1),
      appHeroes: element(by.scss('app-root app-heroes')),
      allHeroes: element.all(by.scss('app-root app-heroes li')),
      heroDetail: element(by.scss('app-root app-hero-detail > div'))
    };
  }
...
async function addToHeroName(text: string): Promise<void> {
  const input = element(by.scss('input'));
  await input.sendKeys(text);
}

async function expectHeading(hLevel: number, expectedText: string): Promise<void> {
  const hTag = `h${hLevel}`;
  const hText = await element(by.scss(hTag)).getText();
  expect(hText).toEqual(expectedText, hTag);
}

function getHeroLiEltById(id: number) {
  const spanForId = element(by.scssContainingText('li span.badge', id.toString()));
